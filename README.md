<p align="center">
  <img src="assets/banner.png" alt="CasaGuard â€” Local AI room security" width="100%" />
</p>
...
<p align="center"><strong>Private, local-first room security for a single USB webcam.</strong><br>CPU-only â€¢ no cloud account â€¢ no subscription â€¢ Docker Compose</p>
...
|---:|---|---|
| ðŸ”’ | **Person detection** | Alerts on people and retains a 30-day event record. |
| ðŸ‘¤ | **Face recognition** | Optional known-face matching in CodeProject.AI. |
| ðŸŽµ | **Audio alerts** | Optional sound classification after a microphone is configured. |
| ðŸ“± | **Mobile notifications** | Frigate browser notifications; use its API with Home Assistant/ntfy for phone delivery. |
| ðŸ§  | **Custom AI models** | Mount compatible CodeProject.AI models in `models/`. |
...
Open `http://YOUR-CASAOS-IP:5000` to finish Frigateâ€™s first-run administrator setup. The first CodeProject.AI start can take several minutes while it initializes.

### Quick start â€” three commands
...
Set `TZ` in `.env` (for example `America/Chicago`) before the final command. The first-login password is deliberately set in Frigateâ€™s browser UI rather than stored in Compose or Git.

## Architecture

```mermaid
flowchart LR
  cam["ðŸ“· Logitech USB webcam\n/dev/video0"] -->|"640Ã—480 Â· 5 fps"| frigate["ðŸ”’ Frigate NVR\nCPU detection + recordings"]
  frigate -->|"events, snapshots & API"| ui["ðŸ–¥ï¸ Frigate UI / browser notifications"]
  frigate -. "snapshot or recording export" .-> cpai["ðŸ§  CodeProject.AI\nface + custom object + audio modules"]
  cpai -->|"match / classification result"| alerts["ðŸ“± Your alert integration\nHome Assistant, ntfy, or webhook bridge"]
```
...
| CPU | 2 modern cores | **AMD Ryzen laptop â€” âœ… Tested & Verified profile** |
| Memory | 4 GB | **8 GB â€” âœ… Tested & Verified profile** |
...
|---|:---:|:---:|:---:|:---:|
| Video stays local | âœ… | âŒ | âŒ | âŒ |
| Required subscription | **No** | Often | Often | Often |
| Open source | âœ… | âŒ | âŒ | âŒ |
| Use your USB webcam | âœ… | âŒ | âŒ | âŒ |
| Works without an internet account | âœ… | âŒ | âŒ | âŒ |
