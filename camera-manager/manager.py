...
def render(cams):
    ...
    for cam in cams.values():
        ...
        name=cam['name'].replace('-','_').replace(' ','_')
        path=cam['path']
        source=f'rtsp://127.0.0.1:8554/{name}'
        detect_fps=min(float(cam.get('fps',5)),5)
        lines += [f'  {name}:','    enabled: true','    ffmpeg:','      inputs:',f'        - path: {source}','          input_args: preset-rtsp-restream','          roles: [detect, record]','    detect:',f'      width: {cam.get("width",640)}',f'      height: {cam.get("height",480)}',f'      fps: {detect_fps:g}','    live:','      streams:',f'        {name}: {name}','    record:','      enabled: true','    snapshots:','      enabled: true']
    return '\n'.join(lines)+'\n'
...
def scan():
    ...
    generated=head.rstrip()+'\n\n'+render(cams)
    if not CONFIG.exists() or CONFIG.read_text()!=generated: CONFIG.write_text(generated)
    try:
